...
KPI 카드, 매출 트렌드 차트, 채널별 비율, 예약 목록을 5초마다 자동 갱신합니다. !!! + merge request 연습중.
...
| 컬럼 | 내용 |
|------|------|
| 예약번호 | `HTL-2605-1000` 형식 고정폭 폰트, **클릭 시 예약 상세 페이지로 이동** |
| 투숙객 | 이름 굵게 표시 |
...
## 예약 상세 페이지 (`/reservation/:id`)

예약번호를 클릭하면 해당 예약의 전체 상세 정보 페이지로 이동합니다.

### 상세 정보 카드 (2×2 그리드)

| 카드 | 표시 정보 |
|------|-----------|
| 예약 기본 정보 | 투숙객명, 연락처, 이메일, 체크인/아웃, 박수 |
| 호텔 정보 | 호텔명, 주소, 객실 타입, 객실 번호 |
| 결제 정보 | 1박 요금, 총 요금(파란색 강조), 예약 채널, 결제 수단, 결제 상태 |
| 특이사항 | 요청사항, 예약 생성일시, 최종 수정일시 |

- 좌상단 **← 예약 목록으로** 버튼으로 대시보드에 복귀
- 페이지 최상단에 예약번호(고정폭 폰트) + 상태 뱃지가 크게 표시
- 로딩 중에는 스피너가 표시되고, 존재하지 않는 예약번호 접근 시 404 안내 메시지

### 투숙객 예약 이력 테이블

상세 카드 하단에 **"OOO님의 예약 이력 (총 N건)"** 섹션이 자동으로 로드됩니다.

- 해당 투숙객의 **전체 예약 이력을 최신순(날짜 내림차순)으로 정렬** 표시
- 투숙객 10명의 고정 데이터 — 현재 예약 2건 + 과거 체류 이력 3~5건 (총 5~7건)
- **현재 보고 있는 예약 행은 파란 배경 + 왼쪽 파란 보더 + "현재" 배지**로 강조
- 이력 테이블의 다른 예약번호 클릭 시 해당 예약 상세 페이지로 바로 이동 (이력도 자동 갱신)
- 별도 로딩 스피너: 상세 카드는 즉시 렌더, 이력은 비동기로 별도 로드

```
예약번호          호텔명         체크인       체크아웃     채널        금액          상태
HTL-2605-1000 현재  파르나스호텔  2026-05-29  2026-05-31  자사몰  360,000원 2박  예약확정
HTL-2605-1010     시그니엘서울  2026-05-26  2026-05-29  Agoda  1,380,000원 3박  예약확정
HTL-2603-1000     시그니엘서울  2026-03-21  2026-03-25  Expedia    600,000원 4박  체크아웃
...
```
...
| **예약 상세** | 예약번호 클릭 → 4가지 정보 카드 + 투숙객 예약 이력 테이블 |
| **투숙객 이력** | 동일 투숙객의 과거·현재·미래 예약 전체 조회, 이력 간 페이지 이동 |
| **반응형** | 모바일 / 태블릿 / 데스크톱 대응 (이력 테이블 가로 스크롤) |
...
| `GET` | `/api/dashboard/reservations` | 최근 예약 20건 (예약번호·투숙객·호텔·채널·금액·상태 등 19개 필드) |
| `GET` | `/api/dashboard/reservations/{id}` | 예약번호 단건 조회 (현재 예약 + 과거 이력 전체 풀에서 검색) |
| `GET` | `/api/dashboard/reservations/guest/{guestName}` | 투숙객 전체 예약 이력 (날짜 내림차순) |
...
└── frontend/
    ...
    └── src/
        ...
        ├── api/dashboardApi.js
        ├── components/
        │   ├── StatCard.jsx / StatCard.css      ← KPI 카드
        │   ├── LineChart.jsx                    ← 매출 트렌드 (AreaChart)
        │   ├── PieChart.jsx                     ← 채널 비율 (도넛)
        │   └── ReservationTable.jsx / .css      ← 예약 목록 (링크 포함)
        └── pages/
            ├── ReservationDetail.jsx            ← 예약 상세 + 투숙객 이력
